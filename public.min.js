...
function(e) {
    ...
    function r(t) {
        t.preventDefault(), e.event.remove(P, "click", r)
    }

    function t(e, r) {
        return (p ? r.originalEvent.touches[0] : r)["page" + e.toUpperCase()]
    }
    ...
    function a(t, a, i) {
        ...
        e.event.trigger(s, {
            originalEvent: t
        }, t.target), s.isDefaultPrevented() && (~a.indexOf("tap") && !p ? e.event.add(P, "click", r) : t.preventDefault()), i && (e.event.remove(P, g + "." + b, o), e.event.remove(P, T + "." + b, n))
    }

    function o(r) {
        if (Y.x = t("x", r), Y.y = t("y", r), Y.dx = Y.x - h.x, Y.dy = Y.y - h.y, Y.adx = Math.abs(Y.dx), Y.ady = Math.abs(Y.dy), i = Y.adx > w.motionThreshold || Y.ady > w.motionThreshold) {
            for (clearTimeout(u), Y.orientation || (Y.adx > Y.ady ? (Y.orientation = "horizontal", Y.direction = Y.dx > 0 ? 1 : -1) : (Y.orientation = "vertical", Y.direction = Y.dy > 0 ? 1 : -1)); r.target && r.target !== h.target;) r.target = r.target.parentNode;
            return r.target !== h.target ? (r.target = h.target, void n.call(this, e.Event(T + "." + b, r))) : void a(r, "drag")
        }
    }

    function n(e) {
        var r, t = e.timeStamp || +new Date,
            o = t - h.time;
        if (clearTimeout(u), i || s || e.target !== h.target) e.target = h.target, o < w.flickDuration && a(e, "flick"), Y.end = !0, r = "drag";
        else {
            var n = y === e.target && t - d < w.doubleTapInterval;
            r = n ? "doubletap" : "tap", y = n ? null : h.target, d = t
        }
        a(e, r, !0)
    }
    var i, s, l, u, y, d, c = navigator.userAgent,
        f = /chrome/i.exec(c),
        m = /android/i.exec(c),
        p = "ontouchstart" in window && !(f && !m),
        v = p ? "touchstart" : "mousedown",
        T = p ? "touchend touchcancel" : "mouseup mouseleave",
    ...
    return e.event.add(P, v + "." + b, function(y) {
        var d = y.timeStamp || +new Date;
        l != d && (l = d, h.x = Y.x = t("x", y), h.y = Y.y = t("y", y), h.time = d, h.target = y.target, Y.orientation = null, Y.end = !1, i = !1, s = !1, u = setTimeout(function() {
            s = !0, a(y, "press")
        }, w.pressDuration), e.event.add(P, g + "." + b, o), e.event.add(P, T + "." + b, n), w.preventDefault && (y.preventDefault(), e.event.add(P, "click", r)))
    }), e.each("tap doubletap press drag flick".split(" "), function(r, t) {
        e.fn[t] = function(e) {
    ...
    jQuery.mbYTPlayer = {
                                        ...
                                        onStateChange: function(r) {
                                            if ("function" == typeof r.target.getPlayerState) {
                                                ...
                                                if (e.preventTrigger || e.isStarting) e.preventTrigger = !1;
                                                    ...
                                                    var a;
                                                    switch (e.state = t, r.data == YT.PlayerState.PLAYING && r.target.setPlaybackQuality(e.opt.quality), t) {
                                                        case -1:
                                                            a = "YTPUnstarted";
                                                            break;
                                                        case 0:
                                                            ...
                                                            break;
                                                        case 1:
                                                            a = "YTPPlay", e.controlBar.length && e.controlBar.find(".mb_YTPPlayPause").html(jQuery.mbYTPlayer.controls.pause), e.isPlayer && e.inlinePlayButton.hide(), jQuery(document).off("mousedown.YTPstart");
                                                            break;
                                                        case 2:
                                                            a = "YTPPause", e.controlBar.length && e.controlBar.find(".mb_YTPPlayPause").html(jQuery.mbYTPlayer.controls.play), e.isPlayer && e.inlinePlayButton.show();
                                                            break;
                                                        case 3:
                                                            e.player.setPlaybackQuality(e.opt.quality), a = "YTPBuffering", e.controlBar.length && e.controlBar.find(".mb_YTPPlayPause").html(jQuery.mbYTPlayer.controls.play);
                                                            break;
                                                        case 5:
                                                            a = "YTPCued"
                                                    }
                                                    var o = jQuery.Event(a);
                                                    o.time = e.currentTime, jQuery(e).trigger(o)
                                                }
                                            }
                                        },
                                        onPlaybackQualityChange: function(r) {
                                            var t = r.target.getPlaybackQuality(),
        ...
        getDataFromAPI: function(e) {
            e.videoData = jQuery.mbStorage.get("YTPlayer_data_" + e.videoID), e.videoData ? (setTimeout(function() {
                e.dataReceived = !0;
                var r = jQuery.Event("YTPChanged");
                r.time = e.currentTime, r.videoId = e.videoID, r.opt = e.opt, jQuery(e).trigger(r);
                var t = jQuery.Event("YTPData");
                for (var a in t.prop = {}, e.videoData) t.prop[a] = e.videoData[a];
                jQuery(e).trigger(t)
            }, e.opt.fadeOnStartTime), e.hasData = !0) : jQuery.mbYTPlayer.apiKey ? jQuery.getJSON(jQuery.mbYTPlayer.locationProtocol + "//www.googleapis.com/youtube/v3/videos?id=" + e.videoID + "&key=" + jQuery.mbYTPlayer.apiKey + "&part=snippet", function(r) {
                e.dataReceived = !0;
                var t, a = jQuery.Event("YTPChanged");
                a.time = e.currentTime, a.videoId = e.videoID, jQuery(e).trigger(a), r.items[0] ? (t = r.items[0].snippet, e.videoData = {}, e.videoData.id = e.videoID, e.videoData.channelTitle = t.channelTitle, e.videoData.title = t.title, e.videoData.description = t.description.length < 400 ? t.description : t.description.substring(0, 400) + " ...", e.videoData.thumb_max = t.thumbnails.maxres ? t.thumbnails.maxres.url : null, e.videoData.thumb_high = t.thumbnails.high ? t.thumbnails.high.url : null, e.videoData.thumb_medium = t.thumbnails.medium ? t.thumbnails.medium.url : null, jQuery.mbStorage.set("YTPlayer_data_" + e.videoID, e.videoData), e.hasData = !0) : (e.videoData = {}, e.hasData = !1);
                var o = jQuery.Event("YTPData");
                for (var n in o.prop = {}, e.videoData) o.prop[n] = e.videoData[n];
                jQuery(e).trigger(o)
            }) : (setTimeout(function() {
                var r = jQuery.Event("YTPChanged");
        ...
        applyMask: function(e) {
            ...
            if (r.off("YTPTime.mask"), e.opt.mask)
                ...
                for (var t in e.opt.mask) e.opt.mask[t] && jQuery("<img/>").attr("src", e.opt.mask[t]);
...
if (jQuery.browser.os = getOS(), jQuery.browser.hasTouch = isTouchSupported(), jQuery.browser.name = navigator.appName, jQuery.browser.fullVersion = "" + parseFloat(navigator.appVersion), jQuery.browser.majorVersion = parseInt(navigator.appVersion, 10), -1 != (verOffset = nAgt.indexOf("Opera"))) jQuery.browser.opera = !0, jQuery.browser.name = "Opera", jQuery.browser.fullVersion = nAgt.substring(verOffset + 6), -1 != (verOffset = nAgt.indexOf("Version")) && (jQuery.browser.fullVersion = nAgt.substring(verOffset + 8));
    ...
    animate: function(e, r, t, a, o) {
        return this.each(function() {
            function n() {
                i.called = !0, i.CSSAIsRunning = !1, s.off(jQuery.CSS.transitionEnd + "." + i.id), clearTimeout(i.timeout), s.css(jQuery.CSS.sfx + "transition", ""), "function" == typeof o && o.apply(i), "function" == typeof i.CSSqueue && (i.CSSqueue(), i.CSSqueue = null)
            }
            var i = this,
                s = jQuery(this);
            ...
            if (i.CSSAIsRunning && i.eventType == l.type && !jQuery.browser.msie && jQuery.browser.version <= 9) i.CSSqueue = function() {
                s.CSSAnimate(e, r, t, a, o)
            };
            else if (i.CSSqueue = null, i.eventType = l.type, 0 !== s.length && e) {
                ...
                if (r || (r = jQuery.fx.speeds._default), "string" == typeof o && (a = o, o = null), jQuery.support.CSStransition) {
                    ...
                    y[a] && (a = y[a]), s.off(jQuery.CSS.transitionEnd + "." + i.id), y = jQuery.CSS.getProp(e);
                    var d = {};
                    jQuery.extend(d, e), d[jQuery.CSS.sfx + "transition-property"] = y, d[jQuery.CSS.sfx + "transition-duration"] = r + "ms", d[jQuery.CSS.sfx + "transition-delay"] = t + "ms", d[jQuery.CSS.sfx + "transition-timing-function"] = a, setTimeout(function() {
                        s.one(jQuery.CSS.transitionEnd + "." + i.id, n), s.css(d)
                    }, 1), i.timeout = setTimeout(function() {
                        i.called || !o ? (i.called = !1, i.CSSAIsRunning = !1) : (s.css(jQuery.CSS.sfx + "transition", ""), o.apply(i), i.CSSAIsRunning = !1, "function" == typeof i.CSSqueue && (i.CSSqueue(), i.CSSqueue = null))
                    }, r + t + 10)
                } else {
                    for (y in e) "transform" === y && delete e[y], "filter" === y && delete e[y], "transform-origin" === y && delete e[y], "auto" === e[y] && delete e[y], "x" === y && (l = e[y], e[u = "left"] = l, delete e[y]), "y" === y && (l = e[y], e[u = "top"] = l, delete e[y]), "-ms-transform" !== y && "-ms-filter" !== y || delete e[y];
                    s.delay(t).animate(e, r, o)
                }
            }
        })
    }
...
var getOS = function() {
        ...
        return -1 != navigator.appVersion.indexOf("Win") && (e.name = "Windows"), -1 != navigator.appVersion.indexOf("Mac") && navigator.appVersion.indexOf("Mobile") < 0 && (e.name = "Mac"), -1 != navigator.appVersion.indexOf("Linux") && (e.name = "Linux"), /Mac OS X/.test(nAgt) && !/Mobile/.test(nAgt) && (e.version = /Mac OS X (10|11[\.\_\d]+)/.exec(nAgt)[1], e.version = e.version.replace(/_/g, ".").substring(0, 5)), /Windows/.test(nAgt) && (e.version = "Unknown.Unknown"), /Windows NT 5.1/.test(nAgt) && (e.version = "5.1"), /Windows NT 6.0/.test(nAgt) && (e.version = "6.0"), /Windows NT 6.1/.test(nAgt) && (e.version = "6.1"), /Windows NT 6.2/.test(nAgt) && (e.version = "6.2"), /Windows NT 10.0/.test(nAgt) && (e.version = "10.0"), /Linux/.test(nAgt) && /Linux/.test(nAgt) && (e.version = "Unknown.Unknown"), e.name = e.name.toLowerCase(), e.major_version = "Unknown", e.minor_version = "Unknown", "Unknown.Unknown" != e.version && (e.major_version = parseFloat(e.version.split(".")[0]), e.minor_version = parseFloat(e.version.split(".")[1])), e
    },
    nameOffset, verOffset, ix;
if (jQuery.browser.os = getOS(), jQuery.browser.hasTouch = isTouchSupported(), jQuery.browser.name = navigator.appName, jQuery.browser.fullVersion = "" + parseFloat(navigator.appVersion), jQuery.browser.majorVersion = parseInt(navigator.appVersion, 10), -1 != (verOffset = nAgt.indexOf("Opera"))) jQuery.browser.opera = !0, jQuery.browser.name = "Opera", jQuery.browser.fullVersion = nAgt.substring(verOffset + 6), -1 != (verOffset = nAgt.indexOf("Version")) && (jQuery.browser.fullVersion = nAgt.substring(verOffset + 8));
else if (-1 != (verOffset = nAgt.indexOf("OPR"))) jQuery.browser.opera = !0, jQuery.browser.name = "Opera", jQuery.browser.fullVersion = nAgt.substring(verOffset + 4);
            ...
            updateSliderVal: function(r) {
                ...
                if (t.opt) {
                    ...
                    var a = e(t).outerWidth(),
                        o = e(t).outerHeight();
                    t.x = "object" == typeof r ? r.clientX + document.body.scrollLeft - this.offset().left : "number" == typeof r ? r * a / t.opt.scale : t.opt.initialval * a / t.opt.scale, t.y = "object" == typeof r ? r.clientY + document.body.scrollTop - this.offset().top : "number" == typeof r ? (t.opt.scale - t.opt.initialval - r) * o / t.opt.scale : t.opt.initialval * o / t.opt.scale, t.y = this.outerHeight() - t.y, t.scaleX = t.x * t.opt.scale / a, t.scaleY = t.y * t.opt.scale / o, t.outOfRangeX = t.scaleX > t.opt.scale ? t.scaleX - t.opt.scale : t.scaleX < 0 ? t.scaleX : 0, t.outOfRangeY = t.scaleY > t.opt.scale ? t.scaleY - t.opt.scale : t.scaleY < 0 ? t.scaleY : 0, t.outOfRange = "h" == t.opt.orientation ? t.outOfRangeX : t.outOfRangeY, t.value = void 0 !== r ? "h" == t.opt.orientation ? t.x >= this.outerWidth() ? t.opt.scale : t.x <= 0 ? 0 : t.scaleX : t.y >= this.outerHeight() ? t.opt.scale : t.y <= 0 ? 0 : t.scaleY : "h" == t.opt.orientation ? t.scaleX : t.scaleY, "h" == t.opt.orientation ? t.level.width(Math.floor(100 * t.x / a) + "%") : t.level.height(Math.floor(100 * t.y / o)), "function" == typeof t.opt.callback && t.opt.callback(t)
                }
            }
    ...
    function(e) {
        ...
        function r(r) {
            var s = e(window),
                l = e("html"),
                u = e("body"),
                y = (document.write, window.SF_Opts);
            r || (r = e(y.alt_menu ? y.alt_menu : "#sfm-nav")), u.prepend(window.SFM_template), (y.alt_menu ? e : setTimeout)(function() {
                var d = function() {
                    function c(e) {
                        if (O) {
                            var r = {
                            ...
                            e && (r.width = ""), I.css(r).removeClass(function(e, r) {
                                var t = r.match(/sfm-custom-active-\d+/);
                                return !!t && t[0]
                            }), I.find(".sfm-active").removeClass("sfm-active")
                        }
                    }

                    function f() {
                        var r, t, a;
                        if ("hidden" === J) return clearTimeout(H), g(), x.addClass("sfm-sidebar-exposed"), C.find(".sfm-navicon-button").add("#sfm-mob-navbar .sfm-navicon-button").addClass("sfm-open"), A.css("visibility", "visible"), u.addClass("sfm-body-pushed"), "full" !== y.sidebar_style && "always" !== y.sidebar_behaviour && x.removeClass("sfm-fully-hidden"), "push" === y.sidebar_behaviour && "full" != y.sidebar_style ? (r = u.children().not("[id*=sfm-], script, style"), t = s.scrollTop(), a = s.scrollLeft(), r.find("*").each(function(e, r) {
                            b(e, r, t, a)
                        })) : "full" === y.sidebar_style && setTimeout(function() {
                            u.addClass("sfm-ui-shown")
                        }, 350), Q && "yes" === y.video_mob || (y.video_bg && "yes" !== y.video_preload ? X && X.YTPGetPlayer() ? X.YTPPlay() : (X = e(".sfm-video").YTPlayer(), window.sfmPlayer = X) : X && X.YTPPlay()), x.removeAttr("aria-hidden"), J = "open", !1
                    }

                    function m() {
                        var e = "sfm-body-pushed sfm-view-pushed-custom";
                        clearTimeout(H), ee = !0, "always" !== y.sidebar_behaviour && "full" !== y.sidebar_style || setTimeout(function() {
                            A.css("visibility", "hidden")
                        }, 800), x.find(".sfm-active-item").removeClass("sfm-active-item"), c(!0), "swipe" !== y.sub_type && (e += " sfm-view-pushed-1 sfm-view-pushed-2 sfm-view-pushed-3 sfm-view-pushed-4 "), u.removeClass(e), x.removeClass("sfm-sidebar-exposed"), C.find(".sfm-navicon-button").add("#sfm-mob-navbar .sfm-navicon-button").removeClass("sfm-open"), x.attr("aria-hidden", "true"), J = "hidden", Z = 0, X && X.YTPPause(), setTimeout(function() {
                            ee = !1, "swipe" === y.sub_type && u.removeClass("sfm-view-pushed-1 sfm-view-pushed-2 sfm-view-pushed-3 sfm-view-pushed-4")
                        }, 250), setTimeout(function() {
                            "full" === y.sidebar_style && u.removeClass("sfm-ui-shown")
                        }, 400)
                    }
                    ...
                    function p(r) {
                        if (!w) {
                            w = setTimeout(function() {
                                w = null
                            }, k), r.stopImmediatePropagation();
                            var t, a, o = e(this);
                            r.type === S ? (t = e(r.target), "yes" === y.parent_ignore && o.is(".sfm-has-child-menu ") || t.closest(".sfm-sm-indicator").length ? (r.preventDefault(), v(o, r)) : function() {
                                var r, t, a = o.find("a"),
                                    n = a.prop("href"),
                                    s = "_blank" === a.attr("target"),
                                    l = a.prop("hash");
                                if (l && l.length > 0 && n == i + l) m(), "#" !== l && ((t = e(l)).length || (t = e('[name="' + l.replace("#", "") + '"]'))), r = t && t.length ? t.offset().top : 0, e("html, body").stop().animate({
                                    scrollTop: r
                                }, 600);
                                else if (s) window.open(n, "_blank");
                                else if ("yes" === y.fade) {
                                    if (-1 !== n.indexOf("#") && "" === l) return;
                                    u.fadeOut(200, function() {
                                        m(), location.href = n
                                    })
                                } else m(), location.href = n
                            }()) : (a = this, "mouseenter" === z && setTimeout(function() {
                                a === $ && v(o, r)
                            }, 52), $ = this)
                        }
                    }
                    ...
                    function v(r, t) {
                        if (!ae && !ee) {
                            clearTimeout(H);
                            var a, o, i, s, l, d, f, m, p, v, T = parseInt((r.closest("ul").attr("class") || "0").match(/\d/)[0]) + 1,
                                b = r.siblings(".sfm-active-item"),
                                P = r.find("a"),
                                h = N,
                                Y = q,
                                w = "";
                            if (c(T <= Z), b.removeClass("sfm-active-item"), r.is(".sfm-has-child-menu"))
                                if (a = r.children(".sfm-child-menu").first(), (l = P.eq(0).find("span").clone()).find("i").remove(), w += '<a href="#" class="sfm-back-parent"><ins class="sfm-sm-indicator"><i></i></ins><span>' + l.text() + "</span></a>", a.length)
                                    if (te < h && "dropdown" !== y.sub_type && "full" !== y.sidebar_style) {
                                        if (u.removeClass("sfm-view-pushed-custom sfm-nav-back sfm-view-pushed-" + T + " sfm-view-pushed-" + (T + 1) + " sfm-view-pushed-" + (T + 2)), !x.is(".sfm-sidebar-exposed") && "always" !== y.sidebar_behaviour) return;
                                        e(".sfm-view-level-" + T).attr("class", function(e, r) {
                                            return r.replace(/(^|\s)sfm-current-\S+/g, "")
                                        }).html('<div class="sfm-scroll-wrapper"><div class="sfm-scroll">' + w + '<ul class="sfm-menu-level-' + T + ' sfm-menu">' + a.html() + "</ul></div></div>").addClass("sfm-current-" + r.data("sfmId")), H = setTimeout(function() {
                                            u.addClass("sfm-view-pushed-" + T + (T <= Z ? " sfm-nav-back" : "")), "always" === y.sidebar_behaviour && A.css("visibility", "visible"), Z = T
                                        }, 25), r.addClass("sfm-active-item")
                                    } else {
                                        o = e(t.target), s = P.attr("href");
                                        var j = P.prop("hash");
                                        ("yes" === y.parent_ignore || o.closest(".sfm-sm-indicator").length || "#" === j || "#" === s || "/" === s) && (r.is(".sfm-submenu-visible") ? P.length && t.type === S && (r.removeClass("sfm-submenu-visible"), a.slideUp({
                                            complete: function() {
                                                setTimeout(function() {
                                                    g(t, "contentChanged", a, "up")
                                                }, 0), document.activeElement && document.activeElement.blur()
                                            }
                                        })) : (n.log("sub", a), r.siblings().filter(".sfm-submenu-visible").removeClass("sfm-submenu-visible").find("> ul").slideUp(), r.addClass("sfm-submenu-visible"), a.slideDown({
                                            start: function() {
                                                g(t, "contentChanged", a)
                                            }
                                        })))
                                    }
                            else if (d = r.attr("data-extra-width"), m = r.attr("data-bg") || "", f = "flyout" === y.sub_type ? parseInt(d || y["width_panel_" + (T + 1)]) : y.width_panel_1, v = r.attr("data-sfm-id"), ("flyout" === y.sub_type ? B[T - 1] + f : y.width_panel_1) < h && "full" !== y.sidebar_style) {
                                if (p = I.find("#sfm-cc-" + r.attr("data-sfm-id")), b.removeClass("sfm-active-item"), u.removeClass("sfm-view-pushed-custom sfm-nav-back sfm-view-pushed-" + T + " sfm-view-pushed-" + (T + 1) + " sfm-view-pushed-" + (T + 2)).addClass("sfm-view-pushed-custom" + (T <= Z ? " sfm-nav-back" : "")), !x.is(".sfm-sidebar-exposed") && "always" !== y.sidebar_behaviour) return;
                                "always" === y.sidebar_behaviour && A.css("visibility", "visible"), r.addClass("sfm-active-item"), i = {
                                    backgroundColor: m,
                                    width: f
                                }, I.css(i).addClass("sfm-custom-active-" + v), I.find(".sfm-back-parent").remove(), I.prepend(e(w)), I.find(".sfm-active").removeClass("sfm-active"), p.width(f), Y > p.outerHeight() ? p.addClass("sfm-vert-align sfm-active") : p.removeClass("sfm-vert-align").addClass("sfm-active"), setTimeout(function() {
                                    n.log("custom content")
                                }, 600), O = !0
                            } else o = e(t.target), ("yes" === y.parent_ignore || o.closest(".sfm-sm-indicator").length || "dropdown" === y.sub_type) && (r.is(".sfm-submenu-visible") || (m = r.attr("data-bg"), r.siblings().filter(".sfm-submenu-visible").removeClass("sfm-submenu-visible").find("> ul").slideUp(), I.addClass("sfm-modal"), i = {
                                opacity: 1,
                                visibility: "visible",
                                backgroundColor: m
                            }, I.find(".sfm-active").removeClass("sfm-active"), I.find("#sfm-cc-" + r.attr("data-sfm-id")).addClass("sfm-active"), I.css(i)));
                            else te < h + 200 && (r.siblings(".sfm-active-item").removeClass("sfm-active-item"), H = setTimeout(function() {
                                u.removeClass("sfm-view-pushed-custom sfm-view-pushed-" + T + " sfm-view-pushed-" + (T + 1) + " sfm-view-pushed-" + (T + 2))
                            }, 50))
                        }
                    }
                    ...
                    function T(r) {
                        if (!w) {
                            w = setTimeout(function() {
                                w = null
                            }, k), r.stopImmediatePropagation(), r.preventDefault();
                            var t = e(this),
                                a = parseInt((t.siblings("ul").attr("class") || "0").match(/\d/)[0]);
                            u.removeClass("sfm-view-pushed-" + a + " sfm-view-pushed-custom"), x.find(".sfm-active-item").removeClass("sfm-active-item"), Z = a - 1
                        }
                    }

                    function g(e, r, t, a) {
                        q = document.documentElement && document.documentElement.clientHeight ? document.documentElement.clientHeight : window.innerHeight ? window.innerHeight : s.height(), N = document.documentElement && document.documentElement.clientWidth ? document.documentElement.clientWidth : window.innerWidth ? window.innerWidth : s.width();
                        var o = 2 * parseInt(y.item_padding),
                            n = j.find(".sfm-search-form"),
                            i = x.find(".sfm-copy");
                        g.cache = g.cache || {
                            headerHeight: V.is(":empty") ? 0 : V.outerHeight() + 70 + o,
                            footerHeight: (_.is(":empty") ? 0 : _.outerHeight()) + (i.length ? i.outerHeight() : 0),
                            contentHeight: j.find(".sfm-menu").outerHeight() + (n.length ? n.outerHeight() : 0) + j.find(".sfm-widget-area").outerHeight()
                        }, r && (g.cache.contentHeight = (t && !a ? t.outerHeight() : "") + j.find(".sfm-menu").outerHeight() + (n.length ? n.outerHeight() : 0));
                        var l = g.cache.contentHeight,
                            u = g.cache.headerHeight,
                            d = g.cache.footerHeight,
                            c = q - l,
                            f = "",
                            m = "";
                        c < u + d + o || "dropdown" === y.sub_type ? "full" !== y.sidebar_style && (f = "sfm-compact", c < u + d ? f += " sfm-compact-header sfm-compact-footer" : m += " sfm-compact-header sfm-compact-footer") : m = "sfm-compact sfm-compact-header sfm-compact-footer", te > N || "dropdown" === y.sub_type || "full" === y.sidebar_style ? (f += " sfm-vertical-nav", z = S) : (m += " sfm-vertical-nav", z = "hover" === y.sub_opening_type ? "mouseenter" : S), m && x.removeClass(m), f && x.addClass(f)
                    }

                    function b(r, t, a, o, n, i) {
                        var l, y, d, c, f, m, p, v, T = e(t);
                        if ("fixed" === T.css("position")) {
                            for (T.addClass("sfm-inner-fixed"), l = T;
                                (l = l.parent()) && !((v = l.css("webkitTransform")) && "none" !== v || l.is("body")););
                            d = (y = l.offset()).left, c = y.top, L && T.is(":visible") && T.hide().data("sfm-ff-hidden", 1), f = {
                                left: T.css("left"),
                                right: T.css("right"),
                                top: T.css("top"),
                                bottom: T.css("bottom")
                    ...
                    function P(r, t) {
                        var a, o, n = e(t);
                        o = {}, (a = n.data("sfm-old-pos")) ? (o[a.toChangeHor] = a[a.toChangeHor], o[a.toChangeVert] = a[a.toChangeVert], "bottom" === a.toChangeVert && (o.top = ""), n.css(o), n.data("sfm-old-pos", "")) : n.css({
                            left: "",
                            top: "",
                            bottom: "",
                            right: ""
                    ...
                    function h(r) {
                        ...
                        for (t in r)
                            if (r.hasOwnProperty(t))
                                if ("skype" === t) e('<li class="sfm-icon-' + t + '"><a href="skype:' + r[t] + '?call"><span>Sk.</span></a></li>').appendTo(_);
                                else if ("email" === t) e('<li class="sfm-icon-' + t + '"><a href="mailto:' + r[t] + '"><span>Em.</span></a></li>').appendTo(_);
                        else {
                            switch (t) {
                                case "facebook":
                                    a = "Fb";
                            ...
                            e('<li class="sfm-icon-' + t + '"><a href="' + r[t] + '" target="_blank"><span>' + a + ".</span></a></li>").appendTo(_)
                        }
                    }

                    function Y() {
                        var e, r, t, a, o;
                        x.add(A).on("touchstart", function(a) {
                            "open" === J && (t = (new Date).getTime(), e = a.originalEvent.touches[0].pageX, r = a.originalEvent.touches[0].clientY)
                        }).on("touchmove", function(e) {
                            "open" === J && (a = e.originalEvent.touches[0].pageX, o = e.originalEvent.touches[0].clientY)
                        }).on("touchend", function() {
                            if ("open" === J) {
                                var n = a > e ? "right" : "left",
                                    i = o - r > 30 || -30 > o - r,
                                    s = a - e > 60 || -60 > a - e;
                                if (!((new Date).getTime() - t > 200 || i) && s) switch (n) {
                                    case "left":
                                        "left" === E ? m() : f();
                                        break;
                                    case "right":
                    ...
                    var w, j, Q = window.SFM_is_mobile,
                        S = Q ? e.mobile ? "vclick" : "tap" : "click",
                        k = y.eventsInterval || 51,
                        C = e(".sfm-rollback"),
                        x = e("#sfm-sidebar"),
                        A = e("#sfm-overlay-wrapper"),
                        _ = e(".sfm-social", x),
                        I = e(".sfm-view-level-custom"),
                        O = !1,
                        V = e(".sfm-logo"),
                        B = [];
                    B.push(parseInt(y.width_panel_1)), B.push(B[0] + parseInt(y.width_panel_2)), B.push(B[1] + parseInt(y.width_panel_3)), B.push(B[2] + parseInt(y.width_panel_4)), y.alt_menu && r.length ? e("#sfm-nav").detach() : r = e("#sfm-nav"), y.$defmenu = r;
                    var M, D, F, E = y.sidebar_pos,
                        R = /msie|trident.*rv\:11\./.test(navigator.userAgent.toLowerCase()),
                        L = /firefox/.test(navigator.userAgent.toLowerCase()),
                        U = (a("transform"), {
                            top: parseInt(l.css("marginTop")),
                            bottom: parseInt(l.css("marginBottom"))
                        }),
                        z = "mouseenter",
                        W = u.css("backgroundImage");
                    "push" === y.sidebar_behaviour && "none" !== W && (u.prepend('<div id="sfm-body-bg"></div>'), D = e("#sfm-body-bg"), ((M = {
                        backgroundColor: u.css("backgroundColor"),
                        backgroundImage: u.css("backgroundImage"),
                        backgroundAttachment: u.css("backgroundAttachment"),
                        backgroundSize: u.css("backgroundSize"),
                        backgroundPosition: u.css("backgroundPosition"),
                        backgroundRepeat: u.css("backgroundRepeat"),
                        backgroundOrigin: u.css("backgroundOrigin"),
                        backgroundClip: u.css("backgroundClip")
                    }).backgroundColor.indexOf("(0, 0, 0, 0") + 1 || M.backgroundColor.indexOf("transparent") + 1) && (M.backgroundColor = "#fff"), F = u.children().not("#sfm-body-bg, #sfm-fixed-container, script, style"), (parseInt(F.first().css("marginTop")) || parseInt(F.last().css("marginBottom"))) && u.addClass("sfm-body-float"), "fixed" === M.backgroundAttachment && (M.position = "fixed", M.backgroundAttachment = "scroll"), D.css(M), t("body > * {position: relative} body {overflow-x:hidden!important}"));
                    var $, H, N, q, X, G = {
                            search: y.search,
                            addHomeLink: "yes" === y.addHomeLink,
                            addHomeText: y.addHomeText || "Home",
                            subMenuSupport: "yes" === y.subMenuSupport,
                            subMenuSelector: y.subMenuSelector,
                            activeClassSelector: y.activeClassSelector || "",
                        ...
                        K = {
                            ...
                            build: function(r) {
                                var t;
                                t = r.clone().removeAttr("id class"), t = this.processDefMenu(t), r.detach(), G.addHomeLink && t.prepend('<li><a href="http://' + window.location.hostname + '">' + G.addHomeText + "</a></li>"), "UL" === t.prop("tagName") ? t.addClass("sfm-menu-level-0") : t.find("ul").first().addClass("sfm-menu-level-0").siblings("ul").addClass("sfm-menu-level-0"), G.subMenuSelector && G.subMenuSupport ? this.buildSubMenus(t) : this.removeSubMenus(t), t.find("a").each(function() {
                                    var r = e(this),
                                        t = r.parent();
                                    r.children("span").length || r.wrapInner(e('<span data-index="' + (t.index() + 1) + '"></span>')), G.subMenuSupport && t.is(".sfm-has-child-menu") && r.append('<ins class="sfm-sm-indicator"><i></i></ins>')
                                }), G.extra && this.attachExtraTo(t.find("[class*=menu-item]")), t.prependTo(".sfm-nav .sfm-va-middle").show(), "yes" === G.search && e(".sfm-va-middle").prepend('<form role="search" method="get" class="sfm-search-form" action="' + y.siteBase + '"><input type="text" class="search-field" placeholder="" value="" name="s"><span></span><input type="submit" class="search-submit" value="Search"></form>'), j = e(".sfm-nav"), t.removeClass("sfm-has-child-menu").addClass("sfm-menu")
                            },
                            processDefMenu: function(r) {
                                var t = G.activeClassSelector ? G.activeClassSelector : "",
                                    a = G.subMenuSelector ? G.subMenuSelector : "",
                                    o = G.allowedTags.replace(/\s/g, "").split(","),
                                    n = r.find("[class*=menu-item]");
                                return r.find('.skip-link, .menu-toggle, a[title*="Skip to content"]').remove(), n.each(function() {
                                    var r = this.id ? this.id.replace("menu-item-", "") : this.className.match(/menu-item-(\d+):?\b/) ? this.className.match(/menu-item-(\d+):?\b/)[1] : "";
                                    e(this).data("sfm-id", r)
                                }), r.find("*").each(function() {
                                    var r = e(this),
                                        n = r.prop("tagName");
                                    r.prop("className");
                                    if (-1 === o.indexOf(n) || "" === e.trim(r.text()) || r.is(".uber-close")) return r.remove();
                                    r.hasClasses(a) ? r.removeAttr("class id").addClass(a.split(",").join(" ")) : r.hasClasses(t) ? r.removeAttr("class id").addClass("sfm-active-class") : r.removeAttr("class id"), r.removeAttr("style"), "LI" === n && r.addClass("sfm-menu-item-" + r.data("sfm-id")).attr("tabindex", "-1")
                                }), r
                            },
                            buildSubMenus: function(r) {
                                for (var t = G.subMenuSelector.replace(/\s/g, "").split(","), a = 0, o = t.length; a < o; a++) r.find("." + t[a]).each(function() {
                                    e(this).removeAttr("id class").addClass("sfm-child-menu ").parent().addClass("sfm-has-child-menu")
                                });
                                this.detectLevel(r)
                            },
                            attachExtraTo: function(r) {
                                var t = "https:" === location.protocol;
                                r.each(function() {
                                    var r, a = e(this),
                                        n = a.data("sfm-id");
                                    if (G.extra[n])
                                        if ((r = o(G.extra[n])).hidemob && Q) a.remove();
                                        else {
                                            if (r.img) "right" == E && "skew" == y.sidebar_style ? a.find("> a").append('<img src="' + (t ? r.img.replace("http:", "https:") : r.img) + '"/>') : a.find("> a").prepend('<img src="' + (t ? r.img.replace("http:", "https:") : r.img) + '"/>');
                                            else if (r.icon) {
                                                var i = r.icon_color ? "color: " + r.icon_color + ";" : "",
                                                    s = r.icon;
                                                "####" === (LAIconManagerUtil.getSet(s) ? LAIconManagerUtil.getSet(s) : "Font Awesome") ? (s = LAIconManagerUtil.getIcon(s), "right" == E && "skew" == y.sidebar_style ? a.find("> a").append('<i style="background-image:url(' + s + ')" class="la_icon la_icon_manager_custom">') : a.find("> a").prepend('<i style="background-image:url(' + s + ')" class="la_icon la_icon_manager_custom">')) : (s = -1 !== s.indexOf("fa-") ? "la" + md5("Font Awesome") + "-" + s.substr(3) : LAIconManagerUtil.getIconClass(s), "right" == E && "skew" == y.sidebar_style ? a.find("> a").append('<i style="' + i + '" class="la_icon ' + s + '">') : a.find("> a").prepend('<i style="' + i + '" class="la_icon ' + s + '">'))
                                            }
                                            r.sline && a.find("> a span").append('<br><i class="sfm-sl">' + r.sline.replace(/\+/g, " ") + "</i>"), r.chapter && a.before('<li class="sfm-chapter"><div>' + r.chapter.replace(/\+/g, " ") + "</div></li>"), r.tabindex && a.attr("tabindex", r.tabindex), r.content && !a.find(".sfm-sm-indicator").length && G.subMenuSupport && (a.addClass("sfm-has-child-menu"), a.find(" > a").append('<ins class="sfm-sm-indicator"><i></i></ins>')), a.data("sfm-extra", r), r.width && a.attr("data-extra-width", r.width), r.bg && a.attr("data-bg", r.bg), a.attr("data-sfm-id", n)
                                        }
                                })
                            },
                            detectLevel: function(r) {
                                r.find(".sfm-child-menu").each(function() {
                                    var r = e(this),
                                        t = r.parents(".sfm-child-menu").length + 1;
                                    r.addClass("sfm-menu-level-" + t), x.find(".sfm-view-level-" + t).length || x.append(e('<div class="sfm-view sfm-view-level-' + t + '"></div>'))
                                })
                            },
                            removeSubMenus: function(r) {
                                if (!G.subMenuSupport) return r.children().each(function() {
                                    e(this).find("ul").remove()
                                });
                                var t = G.subMenuSelector.replace(/\s/g, "").split(",");
                                for (var a in t) r.find("." + t[a]).each(function() {
                                    e(this).remove()
                                })
                            },
                            toggleActiveClasses: function(r) {
                                r.find(".sfm-has-child-menu").each(function() {
                                    var r = e(this);
                                    r.find("*").children(".sfm-active-class").length > 0 && (r.toggleClass("sfm-child-menu-open"), setTimeout(function() {
                                        r.addClass("sfm-child-menu-open"), r.find(".sfm-child-menu").first().show()
                    ...
                    return {
                        init: function(r) {
                            K.build(r), x.parent().is("body") || u.prepend(e("#sfm-body-bg, .sfm-rollback, #sfm-sidebar, #sfm-mob-navbar, #sfm-overlay-wrapper"));
                            var t = Math.min(1 + x.find(".sfm-view").length, 4);
                            if ("flyout" === y.sub_type)
                                for (var a = 1; a <= t; a++) te += parseInt(y["width_panel_" + a]);
                            else te += parseInt(y.width_panel_1);
                            "always" === y.sidebar_behaviour ? x.on("mouseenter " + S, ".sfm-menu li:not(.sfm-chapter)", p) : "full" === y.sidebar_style ? x.on(S, ".sfm-menu li:not(.sfm-chapter)", p) : (x.addClass("sfm-fully-hidden"), x.on(window.transitionEnd, function(r) {
                                if (e(r.target).is(x))
                                    if (x.is(".sfm-sidebar-exposed") && "off" === re) x.on("mouseenter " + S, ".sfm-menu li:not(.sfm-chapter)", p), re = "on";
                                    else if (!x.is(".sfm-sidebar-exposed") && (x.off("mouseenter " + S, ".sfm-menu-level-0 > li, .sfm-view ul > li").addClass("sfm-fully-hidden"), re = "off", A.css("visibility", "hidden"), "push" === y.sidebar_behaviour)) {
                                    var t = e(".sfm-inner-fixed");
                                    R || t.each(P), t.removeClass("sfm-inner-fixed")
                                }
                            })), x.on("keyup", "a", function(e) {
                                13 == e.keyCode && p(e)
                            }), u.on("keyup", function(e) {
                                "open" === J && 27 == e.keyCode && m()
                            }), x.on(S, ".sfm-back-parent", T), e(".sfm-view").mouseenter(function() {
                                clearTimeout(H)
                            }), I.on("hover" === y.sub_opening_type && "dropdown" !== y.sub_type ? "mouseenter" : S, function() {
                                $ = this
                            }), "hover" === y.opening_type ? ("always" !== y.sidebar_behaviour && C.mouseenter(function(r) {
                                e(r.relatedTarget).is(".sfm-sidebar-close") || f()
                            }), x.mouseleave(function(e) {
                                setTimeout(function() {
                                    m()
                                }, 14)
                            })) : "click" !== y.opening_type && "dropdown" !== y.sub_type || "hover" === y.sub_opening_type && x.mouseleave(function(e) {
                                setTimeout(function() {
                                    u.removeClass("sfm-view-pushed-custom sfm-view-pushed-1 sfm-view-pushed-2 sfm-view-pushed-3"), c(!0), x.find(".sfm-active-item").removeClass("sfm-active-item"), "always" === y.sidebar_behaviour && setTimeout(function() {
                                        A.css("visibility", "hidden")
                                    }, 400)
                                }, 250)
                            }), e("#sfm-overlay").on(S, m), y.togglers && !d.run && u.on(S, y.togglers, function(e) {
                                if (!w) return w = setTimeout(function() {
                                    w = null
                                }, k), e.preventDefault(), e.stopImmediatePropagation(), "hidden" === J ? f() : m(), !1
                            }), x.on("click", ".sfm-menu a", function(e) {
                                e.preventDefault()
                            }), d.run || s.on("resize orientationchange", g), x.on("touchmove", function() {
                                ae = !0
                            }), x.on("touchend", function() {
                                ae = !1
                            }), C.find(".sfm-navicon-button").add("#sfm-mob-navbar .sfm-navicon-button, .sfm-sidebar-close").on(S + " keypress", function(r) {
                                if (!w && (w = setTimeout(function() {
                                        w = null
                                    }, k), r.type === S || 13 == r.which)) return "hidden" === J ? (f(), e(this).addClass("sfm-open")) : (m(), e(this).removeClass("sfm-open")), !1
                            }), e("#sfm-sidebar .sfm-search-form span").on(S, function() {
                                e(this).closest("form").trigger("submit")
                            }), Q && (/iPad|iPhone/.test(navigator.userAgent) && x.on("focus", "input", function(r) {
                                u.css("overflowX", "visible"), x.css({
                                    position: "absolute",
                                    top: "-62px"
                                }), e(document).scrollTop(0)
                            }).on("blur", "input", function() {
                                x.css("position", ""), u.css("overflowX", "hidden")
                            }), Y()), h(y.social), "full" === y.sidebar_style && x.find(".sfm-social, .sfm-copy").wrapAll("<div class='sfm-fs-bottom-box'></div>");
                            var o = "full" == y.sidebar_style ? "full" : y.sidebar_behaviour,
                                i = "superfly-on sfm-style-" + y.sidebar_style + " sfm-sidebar-" + o + " sfm-toggle-" + y.opening_type + ("yes" === y.blur ? " sfm-blur" : "");
                            return "side" == y.sidebar_style && (i += " sfm-sub-" + y.sub_type + " sfm-sub-mob-" + y.sub_mob_type), u.addClass(i), d.run || e(document).ajaxComplete(function() {
                                setTimeout(function() {
                                    u.is(".superfly-on") || u.addClass(i)
                                }, 0)
                            }), setTimeout(function() {
                                "always" === y.sidebar_behaviour ? setTimeout(function() {
                                    g(), x.css("opacity", "1")
                                }, 300) : (x.css("opacity", "1"), setTimeout(function() {
                                    x.addClass("sfm-transitions-enabled")
                                }, 300))
                            }, 0), "yes" === y.test_mode && x.addClass("sfm-test-mode"), e(".sfm-close").on(S, function() {
                                I.removeClass("sfm-modal");
                                var e = {
                                    opacity: "",
                                    visibility: ""
                                };
                                I.css(e), I.find(".sfm-active").removeClass("sfm-active")
                            }), e(function() {
                                setTimeout(function() {
                                    var r = e("#sfm-sidebar .wpcf7 form");
                                    r.length && r.each(function() {
                                        var r = e(this);
                                        window.wpcf7 && window.wpcf7.initForm(r), window.wpcf7 && window.wpcf7.cached && window.wpcf7.refill(r)
                                    })
                                }, 0)
                            }), d.run || (e(document).on("sfm-video-player-ready", function() {
                                n.log(" SF video ready ")
                            }), e(document).on("sfm-video-player-error", function() {})), !y.video_bg || "yes" !== y.video_preload && "always" !== y.sidebar_behaviour || Q && "yes" === y.video_mob || (X = e(".sfm-video").YTPlayer(), "always" === y.sidebar_behaviour && X.YTPPlay()), d.run = !0, this
                        },
                        showSidebar: f,
                        hideSidebar: m,
                        sfmPlayer: X,
        ...
        function t(e) {
            if (document.body) {
                ...
                r.type = "text/css", /WebKit|MSIE/i.test(navigator.userAgent) ? r.styleSheet ? r.styleSheet.cssText = e : r.innerText = e : r.innerHTML = e, document.getElementsByTagName("head")[0].appendChild(r)
            } else document.write('<style type="text/css">' + e + "</style>")
        }

        function a(e) {
            var r, t, a = ["Moz", "Webkit", "O", "ms"],
                o = document.createElement("div"),
                n = e.charAt(0).toUpperCase() + e.substr(1);
            if (e in o.style) return e;
            for (t = 0; t < a.length; ++t)
                if ((r = a[t] + n) in o.style) return r;
            this.div = null
        }
        ...
        function o(e) {
            ...
            if ("?" === e.slice(0, 1) && (e = e.slice(1)), "" !== e)
                for (r = e.split("&"), t = 0; t < r.length; t += 1) a = r[t].split("="), o = decodeURIComponent(a[0]), n = a.length > 1 ? decodeURIComponent(a[1]) : void 0, i[o] = n;
            return i
        }
        var n = window.console && window.SF_DEBUG ? window.console : {
                log: function() {}
            },
            i = location.href.replace(/\#.+/, ""),
            s = !1;
        e.fn.hasClasses = function(r) {
            var t = r.replace(/\s/g, "").split(","),
                a = this;
            for (var o in t)
                if (e(a).hasClass(t[o])) return !0;
            return !1
        }, e.fn.addClasses = function(r) {
            var t = r.replace(/\s/g, "").split(","),
                a = this;
            for (var o in t) e(a).addClass(t[o]);
            return this
        }, e(document).one("sfm_doc_body_arrived ready", function() {
            s || (s = !0, r())
        }), e(function() {
            s || (s = !0, r())
        }), e(document).on("sfm_preview_full_refresh", function(t, a, o, n) {
            var i = window.SF_Opts;
            r(n), a && "open" === a && (window.LM.showSidebar(), o && e(".sfm-scroll-main .sfm-scroll").scrollTop(o), "always" !== i.sidebar_behaviour && "full" !== i.sidebar_style && e("#sfm-sidebar").trigger(window.transitionEnd))
        })
    }(window.jQuery);
